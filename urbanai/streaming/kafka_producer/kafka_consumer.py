...
# optional import because not everyone will spin up Kafka during development
try:
    from kafka import KafkaConsumer as KafkaConsumerClient
    from kafka.errors import KafkaError
    kafka_available = True
except ImportError:  # pragma: no cover - missing kafka-python
    KafkaConsumerClient = None  # type: ignore
...
class KafkaConsumer:
    ...
    Handles consuming events from Kafka topics.  When Kafka is not available
    this class simply logs and performs no networking, allowing the rest of the
    system to function without a Kafka cluster.
    """
    ...
    def __init__(
        ...
        if not self.available:
            logger.warning("kafka-python not installed; KafkaConsumer will be a stub")
    ...
    def consume(
        ...
        """
        if not self.available:
            logger.debug("KafkaConsumer stub: consume() called")
            return
        try:
    ...
    def consume_async(
        ...
        if not self.available:
            logger.debug("KafkaConsumer stub: consume_async() called")
            return
