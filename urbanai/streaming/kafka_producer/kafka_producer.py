...
# kafka-python is optional; allow module to import without it for local/dev use
try:
    from kafka import KafkaProducer as KafkaProducerClient
    from kafka.errors import KafkaError
    kafka_available = True
except ImportError:  # pragma: no cover - not installed
    KafkaProducerClient = None  # type: ignore
    KafkaError = Exception  # fallback type
...
class KafkaProducer:
    ...
    Handles publishing events to Kafka topics.  When ``kafka-python`` is not
    installed this class becomes a no‑op stub that simply logs warnings.
    """
    ...
    def __init__(
        ...
        if not self.available:
            logger.warning("kafka-python not installed; KafkaProducer will be a stub")
            # set minimal attributes so other code can access them without crash
    ...
    def publish(
        ...
        """
        if not self.available:
            logger.debug("KafkaProducer stub: publish() called")
            return False
