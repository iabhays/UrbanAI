...
# YOLOv26 research model. torch and ultralytics are optional; wrap
# imports so that the module can be imported even when the machine running
# the code does not have GPU/torch installed (e.g. lightweight test runs).

import warnings

try:
    # some torch builds emit a warning about NumPy compatibility; treat that
    # warning as an error so we can fall back cleanly rather than keep a broken
    # torch module around.
    ...
    # additional check: if torch is older than 2.4 we discourage its use
    try:
        from packaging import version
        if version.parse(torch.__version__) < version.parse("2.4.0"):
            raise ImportError(f"torch version {torch.__version__} is too old")
    except ImportError:
        # packing may not be available; ignore
        pass
except Exception as e:
    # torch could not be imported; create a minimal stand-in module that
    # satisfies attribute lookups used in type hints & subclasses.
    import types
    torch = types.SimpleNamespace(Tensor=object, device=lambda *args, **kwargs: None)
    # create dummy nn namespace with Module attribute so subclassing works
    class _DummyNN:
        Module = object
    nn = _DummyNN()
    F = object
    from urbanai.logging import logger
    logger.warning(f"torch import failed in yolov26 module: {e}")

from typing import Dict, List, Optional, Tuple
from pathlib import Path
import yaml

# ultralytics is imported lazily by the edge detector when needed
YOLO = None
YOLOBase = None

# if torch failed to import earlier we avoid defining any nn.Module subclasses
if torch is None:
    class YOLOv26Detector:
        def __init__(self, *args, **kwargs):
            raise RuntimeError("YOLOv26Detector unavailable because torch could not be imported")
    __all__ = ["YOLOv26Detector"]
